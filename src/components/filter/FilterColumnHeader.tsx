...
const FilterColumnHeader = memo(() => {
  const filterColumnHeaderSelected = useFilterColumnHeaderSelectionStore(
    (state) => state.selected
  );
  const setFilterColumnHeaderSelected = useFilterColumnHeaderSelectionStore(
  ...
  const selectedDisplayName = filterColumnHeaderSelected
    ? lutMapColumns.find(
      (option) => option.columnName === filterColumnHeaderSelected
    )?.displayName
    : "컬럼 선택";
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        ...
        >
          <span className="font-bold truncate">{selectedDisplayName}</span>
              ...
              {lutMapColumns.map((option) => (
                <div
                  ...
                  className={`flex items-center gap-3 px-2 py-1 rounded cursor-pointer transition-colors font-bold 
                                        ${filterColumnHeaderSelected.includes(
                    option.columnName
                  )
                      ? "bg-primary/10"
                      : "hover:bg-accent/50"
