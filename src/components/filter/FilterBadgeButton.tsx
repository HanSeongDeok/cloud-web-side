    ...
    return (
      <>
        {allFilters.map((filter) => (
          ...
          >
            <span className="text-gray-400 font-bold mr-1">{tagId}:</span>
            {filter}
            <Button
...
const AllFilterLutBadge = memo(
  ({ stepClearAll, tagId }: { tagId: string; stepClearAll: () => void }) => {
    return (
      <Badge
        variant="secondary"
...
const FilterBadgeButton = memo(({ pageType }: { pageType: string }) => {
  const filterLutSelected = useFilterLutSelectionStore(
    (state) => state.selected
  );
  // const filterLutClearAll = useFilterLutSelectionStore((state) => state.clearAll);
  const filterColumnHeaderClearAll = useFilterColumnHeaderSelectionStore(
  ...
  const filterLutSetSelected = useFilterLutSelectionStore(
    (state) => state.setSelected
  );

  const removeStep = async (columnHeaderId: string, stepId: string) => {
    const updatedStepSelected = new Map(filterLutSelected).set(
      columnHeaderId,
      filterLutSelected
        .get(columnHeaderId)
        ?.filter((id: string) => id !== stepId) || []
    );
    filterLutSetSelected(updatedStepSelected);

    const mode = convertSearchMode(searchKeyword);
    const searchInfo = await createfilterInfo(
      updatedStepSelected,
      paginationInfo,
      searchKeyword,
      mode
  ...
  return (
    <div>
      {filterLutSelected &&
        Array.from(filterLutSelected.values()).some(
          ...
          <div className="flex flex-wrap gap-2 mt-2 mb-2 overflow-y-auto max-h-40 scrollbar">
            ...
            {Array.from(filterLutSelected.entries()).map(([key, steps]) => (
              <FilterLutBadge
                ...
                columnHeaderId={key}
                tagId={
                  mapColumns.find((opt) => opt.columnName === key)
                    ?.displayName || ""
                }
              />
            ))}
