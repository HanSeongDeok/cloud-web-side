...
const FilterLut = memo(({ pageType }: { pageType: string }) => {
  ...
  const selectAll = () => {
    ...
    const updatedSelected = filterLutSected.get(filterColumnHeaderSelected)?.length === lutMapOptions[filterColumnHeaderSelected]?.length ?
      new Map(filterLutSected).set(filterColumnHeaderSelected, []) :
      new Map(filterLutSected).set(filterColumnHeaderSelected, lutMapOptions[filterColumnHeaderSelected]?.map(option => option.lutValue))
    setFilterLutSectedSelected(updatedSelected);

    const mode = convertSearchMode(searchKeyword);
    const searchInfo = createfilterInfo(updatedSelected, paginationInfo, searchKeyword, mode);
    if (pageType === "storage") {
        ...
        <div className="flex flex-col h-full">
          <div
            className={`flex items-center gap-2 sm:gap-3 px-1 py-1 rounded-lg cursor-pointer transition-colors mb-2
                            ${filterLutSected.get(filterColumnHeaderSelected)?.length === lutMapOptions[filterColumnHeaderSelected]?.length
                ? "bg-primary/10"
                : "hover:bg-gray-200"
              }`}
            onClick={selectAll}
          >
            <Checkbox
              checked={filterLutSected.get(filterColumnHeaderSelected)?.length === lutMapOptions[filterColumnHeaderSelected]?.length || false}
          ...
          <ScrollArea className="h-[calc(100%-3rem)] pr-2">
            <div className="flex flex-col space-y-1 font-bold pr-2 mt-2">
              {filterColumnHeaderSelected && lutMapOptions[filterColumnHeaderSelected]?.map((option) => (
                <div
                  key={option.id}
                  className={`flex items-center gap-3 px-1 py-2 rounded-lg cursor-pointer transition-colors font-bold
                                        ${filterLutSected.get(filterColumnHeaderSelected)?.includes(option.lutValue)
                      ? "bg-primary/10"
                      : "hover:bg-gray-200"
                    }`}
                  onClick={async () => {
                    if (!filterColumnHeaderSelected) return;
                    const updatedSelected = filterLutSected.get(filterColumnHeaderSelected)?.includes(option.lutValue) ?
                      new Map(filterLutSected).set(filterColumnHeaderSelected, filterLutSected.get(filterColumnHeaderSelected)?.filter((item) => item !== option.lutValue) || []) :
                      new Map(filterLutSected).set(filterColumnHeaderSelected, [...filterLutSected.get(filterColumnHeaderSelected) || [], option.lutValue]);
                    ...
                    try {
                      ...
                      const mode = convertSearchMode(searchKeyword);
                      const searchInfo = await createfilterInfo(updatedSelected, paginationInfo, searchKeyword, mode);
                      if (pageType === "storage") {
                        fetchSearchData(searchInfo as SearchInfoBody);
                      } else if (pageType === "trash") {
                        fetchTrashData(searchInfo as SearchInfoBody);
                ...
                >
                  <Checkbox
                    checked={filterLutSected.get(filterColumnHeaderSelected)?.includes(option.lutValue) || false}
                  ...
                  <Label
                    ...
                    style={{
                      color: filterLutSected.get(filterColumnHeaderSelected)?.includes(option.lutValue) ? "#2563eb" : "#222",
                      fontWeight: filterLutSected.get(filterColumnHeaderSelected)?.includes(option.lutValue) ? 700 : 500,
